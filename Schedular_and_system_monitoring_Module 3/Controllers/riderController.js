...
exports.createRider = async (req, res) => {
  try {
    const { name, phone_number, password, status, orders_assigned ,currentLocation,destination} = req.body;
...
exports.updateRider = async (req, res) => {
    try {
      ...
      const { name: newName, phone } = req.body; 
  
      const updatedRider = await Rider.findOneAndUpdate(
        { name: currentName }, // Match by current name
        { name: newName, phone }, // Update name and phone
